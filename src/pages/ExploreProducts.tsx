...
import { decodeProductList, type Product } from '@/lib/product.client'; // Import type if available

// Your frontend mapped product interface
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const response = await fetch(url, {
          method: 'GET',
          // Protobuf requests often don't need specific Accept headers, but standard GET is fine
        });
        ...
        const { products: protobufProducts } = decodeProductList(bytes);
        ...
        const mapped: MappedProduct[] = protobufProducts.map((p, index) => {
           // Ensure numeric ID for your frontend interface
           // If p.id is a string like "123", this parses it. If it's a UUID, you might need to change your interface to string.
           const numericId = parseInt(p.id) || index + 1;

           // Clean up price string (remove currency symbols if they exist in backend data)
           ...
           return {
            ...
            seller: "Gruhini Seller", // You might want to add seller_name to your Protobuf schema later
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {selectedCategory !== "All" && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  {selectedCategory}
                  <button onClick={() => handleCategorySelect("All")} className="ml-1 hover:bg-muted-foreground/20 rounded-full p-0.5">×</button>
                </Badge>
              )}
              {selectedSubcategory && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  {selectedSubcategory}
                  <button onClick={() => setSelectedSubcategory("")} className="ml-1 hover:bg-muted-foreground/20 rounded-full p-0.5">×</button>
                </Badge>
              )}
              {searchTerm && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  "{searchTerm}"
                  <button onClick={() => setSearchTerm("")} className="ml-1 hover:bg-muted-foreground/20 rounded-full p-0.5">×</button>
                </Badge>
              )}
            </div>
          )}
                      ...
                      <div className="relative h-48 bg-gradient-to-br from-accent/20 to-primary/20 rounded-t-lg overflow-hidden">
                        ...
                        {product.badge && (
                          <div className="absolute top-3 left-3">
                            <Badge variant="secondary" className="bg-primary text-primary-foreground text-xs">{product.badge}</Badge>
                          </div>
                        )}
                        {product.verified && (
                          <div className="absolute top-3 right-3">
                            <Badge variant="outline" className="bg-trust-green text-ethnic-primary border-0 text-xs px-2 py-1">✓ Gruhini Verified</Badge>
                          </div>
                        )}
                        ...
                        {product.stock <= 5 && product.stock > 0 && (
                          <div className="absolute bottom-3 left-3">
                            <Badge variant="outline" className="bg-orange-100 text-orange-800 border-orange-200 text-xs">Only {product.stock} left!</Badge>
                          </div>
                        )}
                        {product.stock === 0 && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <Badge variant="outline" className="bg-red-100 text-red-800 border-red-200">Out of Stock</Badge>
                          </div>
                        )}
                      </div>
                      <div className="p-4 flex flex-col flex-grow">
                        <h3 className="font-semibold text-ethnic-primary mb-1 line-clamp-2">{product.name}</h3>
                        <div className="flex items-center text-sm text-muted-foreground mb-2">
                          <Link to={`/seller/${product.seller.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-primary transition-colors flex items-center gap-1">
                            by {product.seller}
                            {isTopSeller && <Star className="h-4 w-4 fill-amber-400 text-amber-500" title="Top Seller" />}
                          </Link>
                        </div>
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <span className="text-lg font-bold text-ethnic-primary">{product.price}</span>
                          {product.originalPrice && (
                            <>
                              <span className="text-sm text-muted-foreground line-through">{product.originalPrice}</span>
                              <span className="text-sm text-primary font-medium">{product.discount}</span>
                            </>
                          )}
                        </div>
                        <div className="flex gap-2 mt-auto pt-3">
                          <Button variant="ethnic" className="w-full" disabled={product.stock === 0}>
                            {product.stock === 0 ? 'OUT OF STOCK' : 'ADD TO CART'}
                          </Button>
                          {product.kitchenVideoUrl && (
                            <Button variant="outline" size="sm" className="w-auto px-3" onClick={() => console.log("View kitchen video:", product.kitchenVideoUrl)}>
